...
void app_bake_card_face(bool32_t has_art, const char* title, const char* corner_label)
{
    ...
    gsi_text(gsi, 24.f, 100.f, corner_label, &g_app.font_label, false, 20, 20, 20, 255);
...
void app_init()
{
    ...
    if (!gs_asset_font_load_from_file("assets/font.otf", &g_app.font_title, 100)) {
        gs_println("WARNING: failed to load assets/font.otf (title, 48pt)");
    }
    if (!gs_asset_font_load_from_file("assets/font.otf", &g_app.font_label, 100)) {
        gs_println("WARNING: failed to load assets/font.otf (label, 24pt)");
    }
